...
router.get('/skin/:nickname/head', (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/skin/:nickname/head.png', (req: Request, res: Response, next: NextFunction) => {
    try {
      const { nickname } = req.params;
      let skinFilePath = path.join(skinPath, `${nickname}.png`);
...
router.get('/docs/skin/:nickname/head.png', (req: Request, res: Response) => {
    const inputString = `## Route: GET /skin/:nickname/head
